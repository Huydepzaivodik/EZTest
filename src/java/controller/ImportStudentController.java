...
@WebServlet("/ImportStudentController")
...
//         List<Integer> ids = excelDAO.importStudentDataFromExcel(excelFilePath);
        List<ImportAccountResult> importAccountResults
                = excelDAO2.importAccountsFromExcel(excelFilePath); // Import Answers from Excel to Database
//        excelDAO.addStudentToStudentClassFromExcel(ids,
//                class_id);//class_id
        excelDAO2.addStudentToStudentClassFromExcel(importAccountResults, class_id);
...
//                .println("<script>alert('" + successMessage + "');window.history.back();</script>");
        String redirectScript = "<script>alert(\"" + escapeJavaScript(successMessage) + "\"); window.location.href='viewListStuInClass?class_id=" + class_id + "';</script>";
